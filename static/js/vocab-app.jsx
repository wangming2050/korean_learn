...
const ALL_WORD_ROWS = 5;

/* ── 离线兜底数据（仅当后端不可用时显示） ── */
const VOCAB_FALLBACK = [
  { id: 1, korean: "책상", chinese: "书桌", pos: "延世1", sentence_korean: "", sentence_chinese: "", audio_url: "" },
  { id: 2, korean: "학생", chinese: "学生", pos: "延世1", sentence_korean: "", sentence_chinese: "", audio_url: "" },
  { id: 3, korean: "의사", chinese: "医生", pos: "延世1", sentence_korean: "", sentence_chinese: "", audio_url: "" },
  { id: 4, korean: "서울", chinese: "首尔", pos: "延世1", sentence_korean: "", sentence_chinese: "", audio_url: "" },
];
...
async function fetchStaticVocab(url = "/static/data/vocab/yonsei1.json") {
  const res = await fetch(url);
  if (!res.ok) throw new Error("本地词汇文件读取失败");
  return res.json();
}

const DEFAULT_BOOKS = [
  { id: "yonsei1", label: "延世韩国语 1", url: "/static/data/vocab/yonsei1.json", count: 0 },
  { id: "yonsei2", label: "延世韩国语 2", url: "/static/data/vocab/yonsei2.json", count: 0 },
  { id: "topik1", label: "TOPIK 1 初级词汇", url: "/static/data/vocab/topik1.json", count: 0 },
  { id: "snu1", label: "首尔大学韩国语第一册", url: "/static/data/vocab/snu1.json", count: 0 },
];
...
function groupVocab(rows) {
  ...
  rows.forEach((r) => {
    if (!map.has(r.id)) {
      map.set(r.id, { id: r.id, ko: r.korean, cn: r.chinese, sourceBook: r.source_book || "", examples: [] });
    }
...
const ChevronDownIcon = () => <Svg><path d="M6 9l6 6 6-6" /></Svg>;

function BookSelector({ books, activeBookId, onSelect }) {
  ...
  const active = books.find((b) => b.id === activeBookId) || books[0] || { label: "词书" };

  return (
    <div className="vocab-book" ref={rootRef}>
      <button
        type="button"
        className="vocab-book-btn"
      ...
      {open && (
        <div className="vocab-book-menu" role="listbox" aria-label="词书列表">
          {books.map((book) => (
            <button
              key={book.id}
              type="button"
              ...
              onClick={() => { setOpen(false); onSelect(book.id); }}
            >
              {book.label}
            </button>
          ))}
...
function VocabApp() {
  ...
  const [activeBookId, setActiveBookId] = useState("yonsei1");
  const [allPage, setAllPage] = useState(1);
  const [allPageSize, setAllPageSize] = useState(25);
  const [allJumpPage, setAllJumpPage] = useState("1");
  ...
  useEffect(() => {
    ...
    (async () => {
      ...
      try {
        setLoading(true);
        const rows = await fetchStaticVocab(activeBook.url);
        if (!alive) return;
        setOffline(false);
        setIndex(0);
        setFlipped(false);
  ...
  useEffect(() => {
    let alive = true;
    fetchJson("/static/data/vocab-books.json")
      .then((manifest) => {
        if (!alive) return;
        const nextBooks = Array.isArray(manifest.books) && manifest.books.length
          ? manifest.books.map((book) => ({
            id: book.id,
            label: book.label || book.title || book.id,
            url: book.url,
            count: book.count || 0,
          }))
          : DEFAULT_BOOKS;
        setBooks(nextBooks);
        if (!nextBooks.some((book) => book.id === activeBookId)) {
          setActiveBookId(nextBooks[0].id);
        }
      })
      .catch(() => setBooks(DEFAULT_BOOKS));
    return () => { alive = false; };
  }, []);
  ...
  useEffect(() => {
    function resetModule(event) {
      if (event.detail?.page !== "vocabulary") return;
      setView("study");
      setIndex(0);
      setFlipped(false);
      setQuiz(null);
  ...
  useEffect(() => {
    ...
    function updatePageSize() {
      const grid = wordListRef.current;
      const tracks = grid ? getComputedStyle(grid).gridTemplateColumns.split(" ").filter(Boolean) : [];
      const columnCount = Math.max(1, tracks.length || 5);
      setAllPageSize(columnCount * ALL_WORD_ROWS);
    }
  ...
  const allTotalPages = Math.max(1, Math.ceil(words.length / allPageSize));

  useEffect(() => {
    setAllPage((page) => Math.min(Math.max(page, 1), allTotalPages));
  }, [allTotalPages]);

  useEffect(() => {
    setAllJumpPage(String(allPage));
  }, [allPage]);
  ...
  function goAllPage(page) {
    setAllPage(Math.min(Math.max(Number(page) || 1, 1), allTotalPages));
  }

  function submitAllPageJump(event) {
    event.preventDefault();
    goAllPage(allJumpPage);
  }
  ...
  if (view === "study") {
    ...
    return (
      <div className="vocab">
        ...
        <BookSelector books={books} activeBookId={activeBookId} onSelect={selectBook} />
  ...
  if (view === "all") {
    ...
    const pageWords = words.slice(start, start + allPageSize);
    return (
      <div className="vocab">
        <BookSelector books={books} activeBookId={activeBookId} onSelect={selectBook} />
        ...
        <div className="wordlist" ref={wordListRef}>
          {pageWords.map((w, i) => (
            <button key={w.id} className="wordlist-item" lang="ko" onClick={() => { setIndex(start + i); setFlipped(false); setView("study"); }}>{w.ko}</button>
          ))}
        </div>
        <div className="wordlist-pager" aria-label="全部单词分页">
          <div className="wordlist-pager-center">
            <button className="wordlist-page-button" type="button" aria-label="首页" disabled={allPage <= 1} onClick={() => goAllPage(1)}>«</button>
            <button className="wordlist-page-button" type="button" aria-label="上一页" disabled={allPage <= 1} onClick={() => goAllPage(allPage - 1)}>‹</button>
            <span className="wordlist-page-status">{allPage} / {allTotalPages}</span>
            <button className="wordlist-page-button" type="button" aria-label="下一页" disabled={allPage >= allTotalPages} onClick={() => goAllPage(allPage + 1)}>›</button>
            <button className="wordlist-page-button" type="button" aria-label="末页" disabled={allPage >= allTotalPages} onClick={() => goAllPage(allTotalPages)}>»</button>
          </div>
          <form className="wordlist-jump" onSubmit={submitAllPageJump}>
            <label htmlFor="wordlistPageJump">跳转</label>
            <input id="wordlistPageJump" type="number" min="1" max={allTotalPages} value={allJumpPage} onChange={(event) => setAllJumpPage(event.target.value)} />
