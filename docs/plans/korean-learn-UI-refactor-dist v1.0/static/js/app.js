    ...
    <div class="section-title">
      ...
      <div style="display:flex;gap:8px;align-items:center;">
        ${!textbookBatchMode ? `
          ...
          <button id="textbookBatchToggle" type="button" style="padding:8px 14px;border-radius:var(--radius-pill);font-size:13px;font-weight:600;">批量管理</button>
        ` : `
          <button id="textbookBatchToggle" type="button" style="padding:8px 14px;border-radius:var(--radius-pill);font-size:13px;font-weight:600;">完成</button>
        `}
      </div>
    </div>
    ${textbookBatchMode ? `
...
function renderPdfAssistantPanel() {
  ...
  expandButton.textContent = pdfAssistantExpanded ? "收起" : "展开";
  ...
  messages.innerHTML = pdfAssistantMessages.length === 0
    ? `<p class="pdf-assistant-empty">可以问“这一页讲了什么？”或“帮我解释这段韩语”。</p>`
    : pdfAssistantMessages.map((message) => `
      <article class="pdf-assistant-message pdf-assistant-message-${escapeHtml(message.role)}">
        ...
        <p>${renderAssistantMessageContent(message.content)}</p>
...
function isLikelyTruncatedAnswer(content) {
  ...
  return !/[。.!?！？」』）)]$/.test(lastChar) || openParenCount > closeParenCount;
}
