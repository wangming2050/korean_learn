...
AI 助教需要后端代理调用模型 API。默认推荐 Gemini API 免费层做 MVP 测试，Railway Variables 里设置：
...
本地开发也可以在项目根目录创建 `.env.ai.local`，服务启动时会自动读取：
...
系统环境变量优先于 `.env.ai.local`，因此 Railway Variables 不会被本地文件覆盖。`.env.ai.local` 已被 `.gitignore` 忽略，不要把真实 API key 写进前端 JS 或提交到 GitHub。
...
- 如果 AI 助教不可用，检查 `AI_PROVIDER`、对应 API key、模型名，以及服务器到 `generativelanguage.googleapis.com` 或 `api.openai.com` 的网络连接。
